...
**Spring Boot Starter，集成 spring-ai-openclaw**
...
**spring-ai-openclaw** 是一个面向 使用 spring-ai-openclaw 的应用 的 Spring Boot Starter，用于将 **spring-ai-openclaw** 集成到 Spring Boot 应用中。它提供自动装配、属性绑定与开箱即用的 Bean，使应用以最小配置即可使用 spring-ai-openclaw 的全部能力。

| 维度 | 说明 |
|---|---|
| 类型 | Spring Boot Starter |
| 消费方 | 使用 spring-ai-openclaw 的 Spring Boot 应用 |
| 核心能力 | 自动装配、属性绑定、开箱即用的 spring-ai-openclaw Bean |
| JDK | `17` |
| 坐标 | `io.github.easy4j:spring-ai-openclaw:3.5.x.20260527-SNAPSHOT` |
| 配置前缀 | `spring.ai.openclaw` |

## 2. 核心能力

| 能力 | 状态 | 说明 |
|---|:---:|---|
| 自动装配 | ✅ 稳定 | 自动注册 spring-ai-openclaw 相关 Bean |
| 属性绑定 | ✅ 稳定 | 绑定 `spring.ai.openclaw.*` 到 `Properties` |
...
| 属性 | 类型 | 默认值 | 必填 | 说明 | 敏感 |
|---|---|---|:---:|---|:---:|
| `spring.ai.openclaw.enabled` | boolean | `true` | 否 | 是否启用 Starter | 否 |
<!-- 更多属性见下方 -->
