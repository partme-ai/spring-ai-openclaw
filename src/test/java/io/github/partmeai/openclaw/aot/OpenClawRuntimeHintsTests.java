...
public class OpenClawRuntimeHintsTests {
	...
	void registerHints() {
		...
		Set<TypeReference> jsonAnnotatedClasses = findJsonAnnotatedClassesInPackage("io.github.partmeai.openclaw");
		...
		assertThat(registeredTypes.contains(TypeReference.of(OpenClawApi.ChatRequest.class))).isTrue();
		assertThat(registeredTypes.contains(TypeReference.of(OpenClawApi.ChatRequest.Tool.class))).isTrue();
		assertThat(registeredTypes.contains(TypeReference.of(OpenClawApi.Message.class))).isTrue();
		assertThat(registeredTypes.contains(TypeReference.of(OpenClawChatOptions.class))).isTrue();
	}
	...
	void registerHintsWithNullClassLoader() {
		...
		org.assertj.core.api.Assertions.assertThatCode(() -> openclawRuntimeHints.registerHints(runtimeHints, null))
	...
	void verifyJsonAnnotatedClassesFromCorrectPackage() {
		...
		for (TypeReference classRef : jsonAnnotatedClasses) {
			assertThat(classRef.getName()).startsWith("io.github.partmeai.openclaw");
		}
	}
	...
	void verifyHintsRegistrationWithCustomClassLoader() {
		...
		org.assertj.core.api.Assertions
			.assertThatCode(() -> openclawRuntimeHints.registerHints(runtimeHints, customClassLoader))
			.doesNotThrowAnyException();
